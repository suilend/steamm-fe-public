...
import { CheckCircle, ChevronDown, ChevronUp, ExternalLink, Link, XCircle } from "lucide-react";
import { useSettingsContext } from "@suilend/frontend-sui-next";
import { cn } from "@/lib/utils";
import { DEFAULT_CONFIG, TokenCreationStatus, useLaunch } from "@/contexts/LaunchContext";
...
export default function TokenCreationContent() {
  ...
  const hasAcknowledgedOrInProgress = hasAcknowledged || status !== TokenCreationStatus.Pending;
  ...
  if (status === TokenCreationStatus.Success) {
    ...
    return (
      <div className="flex flex-col items-center gap-4 p-4 text-center sm:p-6">
        ...
        <p className="text-sm text-muted-foreground">
          We encountered an error during the token creation process. Your progress has been saved. Please continue your
        </p>
  ...
  return <div className={cn("flex w-full flex-col gap-4")}>
...
{config.iconUrl && (
    <div className="flex items-center flex-col">
      <img
        src={config.iconUrl}
        alt="Token icon"
        className="h-12 w-12 object-contain rounded-full overflow-hidden"
      />
    </div>
)}
  ...
  <Separator />
      ...
      {showAdvancedOptions && <>
          <div className="flex justify-between">
            <span className="text-sm font-medium">Initial token supplied to LP</span>
            <span className="text-sm">{Number(config.initialSupply) / 10} {config.tokenSymbol}</span>
          </div>
          <div className="flex justify-between">
