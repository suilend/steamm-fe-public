...
export default function TokenCreationProgress() {
  ...
  const steps = function(initialSupply: string, tokenSymbol: string) {
    ...
    return (
      <div className="flex flex-col items-center space-y-4 py-8 text-center w-full">
          ...
          <div className="relative flex w-full justify-between gap-2">
            {steps(initialSupply, tokenSymbol).map((step, index) => {
              ...
              if (step.status === status && txnInProgress) {
                circle = <div className="absolute inset-0 h-full w-full rounded-full border-2 border-border border-t-foreground animate-spin"/>
              ...
              <div key={step.status} className="flex flex-col items-center">
                ...
                {transactionDigests[step.status] && transactionDigests[step.status].map((digest) => (
                  <a
                    href={explorer.buildTxUrl(digest)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-secondary-foreground flex underline items-center"
                  >
                    View transaction
              ...
              return index === 0 ? [acc, step] : [acc, <div
                className={cn(
                  "h-[2px] w-16 -translate-y-1/2 mt-5",
                  
                  index < status ? "bg-gradient-to-r from-border bg-success" : "bg-gradient-to-r from-border to-foreground"
        ...
        {txnInProgress && <p className="text-sm text-secondary-foreground mt-6">
