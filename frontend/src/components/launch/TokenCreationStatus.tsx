...
export default function TokenCreationStatus({
  ...
  if (status === "pending" || status === "publishing" || status === "minting") {
    ...
    if (status === "publishing") {
      ...
      firstStepContent = (
        <div className="border-t-transparent rounded-full h-5 w-5 animate-spin border-2 border-background"></div>
      );
    ...
    return (
      <div className="flex flex-col items-center space-y-4 p-4 text-center sm:p-6">
        <div className="relative h-12 w-12">
          <div className="rounded-full absolute inset-0 h-full w-full animate-spin border-4 border-border border-t-primary"></div>
        </div>
