  ...
  if (!moduleInitPromise) {
    moduleInitPromise = (async () => {
      try {
        ...
        throw new Error('Failed to initialize token creation module. Please refresh the page and try again.');
      }
    })();
  }
...
const generateTokenBytecode = async (params: CreateTokenParams): Promise<Uint8Array> => {
  try {
    ...
    updated = update_constants(
      updated,
      bcs.string().serialize(params.description || "").toBytes(),
...
const useCreateToken = () => {
  ...
  const [creationStatus, setCreationStatus] = useState<"idle" | "publishing" | "minting" | "success" | "error">("idle");

  const createToken = async (params: CreateTokenParams): Promise<CreateTokenResult | null> => {
    if (!address) {
      showErrorToast("Wallet not connected", new Error("Please connect your wallet"), undefined, true);
      return null;
    }
    ...
    try {
      ...
      if (!treasuryCapChange || treasuryCapChange.type !== "created" || 
          !coinMetadataChange || coinMetadataChange.type !== "created") {
        throw new Error("Failed to find created token objects in transaction results");
      }
      
      // Extract the token type from the TreasuryCap object type
      const tokenType = treasuryCapChange.objectType.split("<")[1]?.split(">")[0] || "";
      ...
      if (params.initialSupply && parseFloat(params.initialSupply) > 0) {
        try {
          ...
          const initialSupplyAmount = BigInt(
            Math.floor(parseFloat(params.initialSupply) * Math.pow(10, params.decimals))
          );
