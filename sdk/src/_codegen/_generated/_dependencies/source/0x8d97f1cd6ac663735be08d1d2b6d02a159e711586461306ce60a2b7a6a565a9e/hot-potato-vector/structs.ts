...
export class HotPotatoVector<T extends TypeArgument> implements StructClass {
    ...
    static reified<T extends Reified<TypeArgument, any>>(
        ...
        return {
            ...
            fromFields: (fields: Record<string, any>) =>
                HotPotatoVector.fromFields(
                    T,
                    fields,
                ),
            fromFieldsWithTypes: (item: FieldsWithTypes) =>
                HotPotatoVector.fromFieldsWithTypes(
                    T,
                    item,
                ),
            fromBcs: (data: Uint8Array) =>
                HotPotatoVector.fromBcs(
                    T,
                    data,
                ),
            bcs: HotPotatoVector.bcs(toBcs(T)),
            fromJSONField: (field: any) =>
                HotPotatoVector.fromJSONField(
                    T,
                    field,
                ),
            fromJSON: (json: Record<string, any>) =>
                HotPotatoVector.fromJSON(
                    T,
                    json,
                ),
            fromSuiParsedData: (content: SuiParsedData) =>
                HotPotatoVector.fromSuiParsedData(
                    T,
                    content,
                ),
            fromSuiObjectData: (content: SuiObjectData) =>
                HotPotatoVector.fromSuiObjectData(
                    T,
                    content,
                ),
            fetch: async (client: SuiJsonRpcClient, id: string) => HotPotatoVector.fetch(
                client,
                T,
                id,
            ),
            new: (
                fields: HotPotatoVectorFields<ToTypeArgument<T>>,
            ) => {
    ...
    static phantom<T extends Reified<TypeArgument, any>>(
        ...
        return phantom(HotPotatoVector.reified(
            T
        ));
    }
    ...
    static get bcs() {
        return <T extends BcsType<any>>(T: T) => bcs.struct(`HotPotatoVector<${T.name}>`, {
            contents:
                bcs.vector(T)
    ...
    static fromJSON<T extends Reified<TypeArgument, any>>(
        ...
        assertReifiedTypeArgsMatch(
            composeSuiType(HotPotatoVector.$typeName,
            extractType(typeArg)),
            json.$typeArgs,
            [typeArg],
        )

        return HotPotatoVector.fromJSONField(
            typeArg,
            json,
        )
    }
    ...
    static fromSuiParsedData<T extends Reified<TypeArgument, any>>(
        ...
        return HotPotatoVector.fromFieldsWithTypes(
            typeArg,
            content
        );
    }
    ...
    static fromSuiObjectData<T extends Reified<TypeArgument, any>>(
        ...
        if (data.bcs) {
            ...
            return HotPotatoVector.fromBcs(
                typeArg,
                fromBase64(data.bcs.bcsBytes)
            );
        }
        if (data.content) {
            return HotPotatoVector.fromSuiParsedData(
                typeArg,
                data.content
            )
        }
    ...
    static async fetch<T extends Reified<TypeArgument, any>>(
        ...
        return HotPotatoVector.fromSuiObjectData(
            typeArg,
            res.data
        );
    }
