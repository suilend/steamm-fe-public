...
export interface SetFields<A extends TypeArgument> {
    keys: ToField<Vector<A>>; elems: ToField<Table<ToPhantom<A>, ToPhantom<Unit>>>
}

export type SetReified<A extends TypeArgument> = Reified<
    Set<A>,
    SetFields<A>
>;
...
export class Set<A extends TypeArgument> implements StructClass {
    ...
    readonly keys:
        ToField<Vector<A>>
    ; readonly elems:
        ToField<Table<ToPhantom<A>, ToPhantom<Unit>>>
    ...
    static reified<A extends Reified<TypeArgument, any>>(
        ...
        return {
            ...
            fromSuiParsedData: (content: SuiParsedData) =>
                Set.fromSuiParsedData(
                    A,
                    content,
                ),
            fromSuiObjectData: (content: SuiObjectData) =>
                Set.fromSuiObjectData(
                    A,
                    content,
                ),
            ...
            new: (
                fields: SetFields<ToTypeArgument<A>>,
            ) => {
                return new Set(
                    [extractType(A)],
                    fields
    ...
    static get bcs() {
        return <A extends BcsType<any>>(A: A) => bcs.struct(`Set<${A.name}>`, {
            keys:
                bcs.vector(A)
            , elems:
                Table.bcs
    ...
    static fromFields<A extends Reified<TypeArgument, any>>(
        typeArg: A, fields: Record<string, any>
    ): Set<ToTypeArgument<A>> {
        return Set.reified(
            typeArg,
        ).new(
            {keys: decodeFromFields(reified.vector(typeArg), fields.keys), elems: decodeFromFields(Table.reified(reified.phantom(typeArg), reified.phantom(Unit.reified())), fields.elems)}
        )
    }

    static fromFieldsWithTypes<A extends Reified<TypeArgument, any>>(
        typeArg: A, item: FieldsWithTypes
    ): Set<ToTypeArgument<A>> {
        if (!isSet(item.type)) {
        ...
        return Set.reified(
            ...
            {keys: decodeFromFieldsWithTypes(reified.vector(typeArg), item.fields.keys), elems: decodeFromFieldsWithTypes(Table.reified(reified.phantom(typeArg), reified.phantom(Unit.reified())), item.fields.elems)}
        )
    }

    static fromBcs<A extends Reified<TypeArgument, any>>(
        typeArg: A, data: Uint8Array
    ): Set<ToTypeArgument<A>> {
        const typeArgs = [typeArg];

        return Set.fromFields(
            typeArg,
            Set.bcs(toBcs(typeArgs[0])).parse(data)
        )
    }
    ...
    static fromJSONField<A extends Reified<TypeArgument, any>>(
        ...
        return Set.reified(
            ...
            {keys: decodeFromJSONField(reified.vector(typeArg), field.keys), elems: decodeFromJSONField(Table.reified(reified.phantom(typeArg), reified.phantom(Unit.reified())), field.elems)}
        )
    }

    static fromJSON<A extends Reified<TypeArgument, any>>(
        typeArg: A, json: Record<string, any>
    ): Set<ToTypeArgument<A>> {
        if (json.$typeName !== Set.$typeName) {
            throw new Error("not a WithTwoGenerics json object")
        };
        assertReifiedTypeArgsMatch(
            composeSuiType(Set.$typeName,
            extractType(typeArg)),
            json.$typeArgs,
            [typeArg],
        )

        return Set.fromJSONField(
            typeArg,
            json,
        )
    }
    ...
    static fromSuiParsedData<A extends Reified<TypeArgument, any>>(
        typeArg: A, content: SuiParsedData
    ): Set<ToTypeArgument<A>> {
        ...
        return Set.fromFieldsWithTypes(
            typeArg,
            content
        );
    }
    ...
    static async fetch<A extends Reified<TypeArgument, any>>(
        client: SuiJsonRpcClient, typeArg: A, id: string
...
export type UnitReified = Reified<
    Unit,
    UnitFields
>;
...
export class Unit implements StructClass {
    ...
    static reified(): UnitReified {
        return {
            ...
            fromSuiParsedData: (content: SuiParsedData) =>
                Unit.fromSuiParsedData(
                    content,
                ),
            fromSuiObjectData: (content: SuiObjectData) =>
                Unit.fromSuiObjectData(
                    content,
                ),
            fetch: async (client: SuiJsonRpcClient, id: string) => Unit.fetch(
                client,
                id,
            ),
            new: (
    ...
    static fromSuiParsedData(
        ...
        return Unit.fromFieldsWithTypes(
            content
        );
    }
