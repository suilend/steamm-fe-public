...
export function isEvent(type: string): boolean {
    ...
    return type.startsWith(`${PKG_V1}::script_events::Event` + '<');
}
...
export class Event<T extends TypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V1}::script_events::Event`;
    static readonly $numTypeParams = 1;
    static readonly $isPhantom = [false,] as const;

    readonly $typeName = Event.$typeName;
    readonly $fullTypeName: `${typeof PKG_V1}::script_events::Event<${ToTypeStr<T>}>`;
    ...
    private constructor(typeArgs: [ToTypeStr<T>], fields: EventFields<T>,
        ...
        this.$fullTypeName = composeSuiType(
            Event.$typeName,
            ...typeArgs
        ) as `${typeof PKG_V1}::script_events::Event<${ToTypeStr<T>}>`;
    ...
    static reified<T extends Reified<TypeArgument, any>>(
        ...
        return {
            ...
            fullTypeName: composeSuiType(
                Event.$typeName,
                ...[extractType(T)]
            ) as `${typeof PKG_V1}::script_events::Event<${ToTypeStr<ToTypeArgument<T>>}>`,
            typeArgs: [
                extractType(T)
            ...
            fromSuiParsedData: (content: SuiParsedData) =>
                Event.fromSuiParsedData(
                    T,
                    content,
                ),
            fromSuiObjectData: (content: SuiObjectData) =>
                Event.fromSuiObjectData(
                    T,
                    content,
                ),
            ...
            new: (
                fields: EventFields<ToTypeArgument<T>>,
            ) => {
                return new Event(
                    [extractType(T)],
                    fields
    ...
    static get bcs() {
        return <T extends BcsType<any>>(T: T) => bcs.struct(`Event<${T.name}>`, {
            event:
                T
    ...
    static fromBcs<T extends Reified<TypeArgument, any>>(
        ...
        return Event.fromFields(
            typeArg,
            Event.bcs(toBcs(typeArgs[0])).parse(data)
        )
    }
    ...
    static fromJSON<T extends Reified<TypeArgument, any>>(
        ...
        assertReifiedTypeArgsMatch(
            composeSuiType(Event.$typeName,
            extractType(typeArg)),
            json.$typeArgs,
            [typeArg],
        )

        return Event.fromJSONField(
            typeArg,
            json,
        )
    }
    ...
    static fromSuiParsedData<T extends Reified<TypeArgument, any>>(
        ...
        return Event.fromFieldsWithTypes(
            typeArg,
            content
        );
    }
    ...
    static fromSuiObjectData<T extends Reified<TypeArgument, any>>(
        ...
        if (data.content) {
            return Event.fromSuiParsedData(
                typeArg,
                data.content
            )
        }
    ...
    static async fetch<T extends Reified<TypeArgument, any>>(
        ...
        return Event.fromSuiObjectData(
            typeArg,
            res.data
        );
    }
