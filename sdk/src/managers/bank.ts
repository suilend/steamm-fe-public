...
import { Codegen } from "..";
import { InitLendingArgs, createBank } from "../abis";
import { castNeedsRebalance } from "../abis/bank/bankTypes";
...
export class BankManager implements IManager {
  ...
  public async initLending(
    ...
    const banks = await this.sdk.fetchBankData();
  ...
  public async getTotalFunds(bankInfo: BankInfo): Promise<bigint> {
    ...
    Codegen.EmitDryRun.emitEvent(
      tx,
      "u64",
      totalFunds,
      this.sdk.scriptInfo.publishedAt,
    );
  ...
  public async queryRebalance(
    ...
    const banks = Object.values(await this.sdk.fetchBankData());
  ...
  public async createBank(
    ...
    createBank(tx, callArgs, this.sdk.steammInfo);
  }
  ...
  private async batchQueryRebalance(
    ...
    for (const bankInfo of banks) {
      ...
      Codegen.BankScript.needsRebalance(
        tx,
        bank.typeArgs(),
        {
          bank: bank.bank(tx),
