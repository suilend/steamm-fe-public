...
import { Codegen } from "..";
import { BankAbi, BankScript } from "../abis";
import { MultiSwapQuote, castMultiSwapQuote } from "../abis/pool/poolTypes";
...
import { getOracleArgs, getOracleV2Args } from "./pool";
...
export class Router implements IManager {
  ...
  public async swapWithRoute(
    ...
    for (const hop of args.route) {
      ...
      const bankA = new BankAbi(this.sdk.steammInfo, bankInfos[coinAIndex]);
      const bankB = new BankAbi(this.sdk.steammInfo, bankInfos[coinBIndex]);
      ...
      const amountIn =
        i === 0
          ...
          : Codegen.Pool.swapResultAmountOut(
              tx,
              swapResults[i - 1],
              this.sdk.sdkOptions.packages.steamm.publishedAt,
            );
  ...
  public async quoteSwapRoute(
    ...
    const bankX = new BankAbi(this.sdk.steammInfo, bankInfoX);
    const bankY = new BankAbi(this.sdk.steammInfo, bankInfoY);
    ...
    for (const hop of route) {
      ...
      const amountOut = Codegen.Quote.amountOut(
        tx,
        quote,
        this.sdk.sdkOptions.packages.steamm.publishedAt,
      );
    ...
    const bankScript = this.bankScript(bankInfoX, bankInfoY);
  ...
  private bankScript(bankInfoA: BankInfo, bankInfoB: BankInfo): BankScript {
  ...
  public mintBTokens(
    ...
    for (const coinType of coinTypes) {
      ...
      const bToken =
        i === 0
          ? (() => {
              const bank = new BankAbi(this.sdk.steammInfo, bankInfo);
  ...
  public collectBTokenDust(
    ...
    const bankIn = new BankAbi(this.sdk.steammInfo, bankInfo);
  ...
  public destroyOrTransfer(
    ...
    return tx.moveCall({
      target: `${this.sdk.scriptInfo.publishedAt}::pool_script::destroy_or_transfer`,
